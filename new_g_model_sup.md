...
I_t <- N*t(Q[range_tau>=min_val,])%*%D_at%*%t(P_t[,range_tau>=min_val])%*%solve(diag(as.vector(P_t[,range_tau>=min_val]%*%g_til)))%*%P_t[,range_tau>=min_val]%*%D_at%*%Q[range_tau>=min_val,]

s_dots <-(1/norm(a,"2"))*(diag(x=1,length(a),length(a))-(a%*%t(a)/(norm(a,"2"))^2)) 
cov_a <- solve(I+s_dots)%*%I%*%solve(I+s_dots)
cov_at <- solve(I_t+s_dots)%*%I_t%*%solve(I_t+s_dots)

my_se_t <- D_at%*%Q[range_tau>=min_val,]%*%cov_at%*%t(Q[range_tau>=min_val,])%*%D_at
...
ressi<-SE_simolation(n_sim=10,c=1,n=10000,rt=-1)
theo_simo_data <-SE_simulation_Theo(n_sim=10,c=1,n=10000,rt=-1)
c1 <- apply(ressi$g_tilde,2,sd)
c2 <-colMeans(data.frame(theo_simo_data[1]))
plot_df <- data.frame(cbind(range_tau[range_tau>=-1],c2,c1))
colnames(plot_df)<-c("Theta","theo_g_tilde","simo_g_tilde")
ggplot(plot_df)+geom_line(aes(x=Theta,y=theo_g_tilde,color="Theoratical SE")) + geom_point(aes(x=Theta,y=simo_g_tilde,color="Simulation SE"))+ggtitle(TeX("Simulation SE vs Theoratical SE of \\tilde{g}(\\alpha)")) + ylab("SE")+xlab(TeX("\\theta"))+theme_bw() +theme(legend.position = "bottom", legend.title = element_blank())
```
